...
def perform_friedman_test(episodes_dir, baselines_dir, plot_dir, instance):
    ...
    for problem_index in range(1, 25):  # 1 to 24
        # Load the data for episode_1200, cma_es, and one_fifth_es
        ep_1200_data = load_data(os.path.join(episodes_dir,
                                              f'fitness_episode_1200_problem_{problem_index}_instance_{instance}.npy'))
        ...
        combined_data = np.stack((ep_1200_data[:, -1], cma_es_data[:, -1], one_fifth_es_data[:, -1]), axis=-1)
        ...
        mean_fitness_episode_1200 = np.mean(ep_1200_data)
        ...
        best_mean_fitness = min(mean_fitness_episode_1200, mean_fitness_cma_es, mean_fitness_one_fifth_es)

        # Append the results to the list
        full_results.append([
            problem_index,  # Problem number
            average_ranks_problem[0],  # Avg Rank for episode_1200
    ...
    results_df = pd.DataFrame(full_results, columns=[
        'Problem Number', 'Avg Rank (episode_1200)', 'Avg Rank (cma_es)', 'Avg Rank (one_fifth_es)',
    ...
    mean_rank_ep_1200 = np.mean(all_ranks[:, 0])
    ...
    overall_row = {
        'Problem Number': 'Mean',
        'Avg Rank (episode_1200)': mean_rank_ep_1200,
...
def format_latex_table(df):
    ...
    for idx, row in df.iterrows():
        ...
        ranks = [row['Avg Rank (episode_1200)'], row['Avg Rank (cma_es)'], row['Avg Rank (one_fifth_es)']]
        min_rank = min(ranks)
    ...
    mean_ranks = [df[f'Avg Rank (episode_1200)'].mean(), df[f'Avg Rank (cma_es)'].mean(),
